...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        # 扩展hidden以匹配encoder_outputs的维度
        hidden = hidden.unsqueeze(1).repeat(1, seq_len, 1)  # [batch_size, seq_len, hidden_size]
        #hidden = hidden.unsqueeze(1).expand(-1, seq_len, -1)  # [batch_size, seq_len, hidden_dim]
        # 计算能量值
        energy = torch.tanh(self.attn(torch.cat((hidden, encoder_outputs), dim=2)))  # [batch_size, seq_len, hidden_size]
        
        # 使用参数向量计算得分
        v = self.v.unsqueeze(0).unsqueeze(0).repeat(encoder_outputs.size(0), 1, 1)  # [batch_size, 1, hidden_size]
        attention = torch.bmm(v, energy.transpose(1, 2)).squeeze(1)  # [batch_size, seq_len]
...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hidden_dim, num_layers, dropout,return_method='concat'):
        ...
        # 定义参数method是用来选择前向和后向的隐藏状态的融合方式
        self.method = return_method
        self.num_layers = num_layers
        # 定义参数embedding作用是将输入的单词转换为向量
        self.embedding = nn.Embedding(input_dim, emb_dim)
        # 定义GRU是编码器的核心,做为循环神经网络
        self.gru = nn.GRU(emb_dim, hidden_dim, num_layers,bidirectional=True,batch_first=True, dropout=dropout)
        # 这里的dropout是指在RNN层之间添加dropout,防止过拟合
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        # x的维度是(batch_size, seq_len)
        embedded = self.dropout(self.embedding(x))
        # embedded的维度是(batch_size, seq_len, emb_dim)
        
        outputs, hidden = self.gru(embedded)
        # 处理双向GRU的隐藏状态
        # 分离前向和后向的隐藏状态
        forward_hidden = hidden[:self.num_layers]
        backward_hidden = hidden[self.num_layers:]
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden,encoder_outputs):
        ...
        if self.return_method == 'concat':
            attn_hidden = torch.cat((hidden[-1], hidden[-2]), dim=1)
        ...
        # 返回预测值,隐藏状态和注意力权重
        print(f"Decoder output shape : {prediction.shape}")
...
def preprocess_data(src_sentences, tgt_sentences, word2idx, idx2word,max_len=50):
    '''文本数据转为索引'''
    src_data = []
    tgt_data = []
...
def main():
    ...
    print(f'使用设备: {device}')
    print('加载数据...')
    train_loader, test_loader,word2idx,idx2word,vocab_size= load_data('党金虎/week08/chinese_test_data/couplet/',50)  # 假设load_data函数已经定义
    print('数据加载完成...')
    ...
    # TensorBoard
    writer = SummaryWriter(log_dir='党金虎/week08/runs')
    # 训练模型
    print("开始训练...")
    num_epochs = 10
    for epoch in range(num_epochs):
        ...
        # 保存最佳模型
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), f'党金虎/week08/model_epoch_{epoch+1}.pth')
            print(f'模型已保存: model_epoch_{epoch+1}.pth')
        
    # 根据模型测试一些样例
    model.load_state_dict(torch.load('党金虎/week08/runs/model_epoch_10.pth'))
